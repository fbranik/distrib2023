        ...
        if request.method == 'PUT':
            ...
            for id, tableInfoDict in payload['myNodesTable'].items():
                if int(id) not in myNode.nodesTable:
                    myNode.syncNodesTable(int(id), tableInfoDict['walletAddress'], tableInfoDict['walletBalance'], tableInfoDict['ip'], tableInfoDict['port'],payload['utxo'])
                    if myNode.Id == 0 and int(id) != 0:
                        ...
                        thread = threading.Thread(target=broadCastFirstTransaction, kwargs={'firstTransaction': firstTransaction})
                        thread.start()
